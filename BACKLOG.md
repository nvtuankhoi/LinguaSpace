...
> **Note (2026-07-13):** đã scan api-contract vs FE hệ thống — **các gap FE-only sạch đã gần cạn**. Các mục còn lại trong C/D/E đều cần backend work, external config/SDK, hoặc quyết định contract ngoài (FCM cần FCM SDK, Google OAuth cần `GoogleClientId` + SDK, VocabCard/Poll/media post types cần xác nhận contract, screen-share/active-speaker UI là effort lớn video). Backend-only sẵn sàng làm: **Resend verification email sender** (`ResendEmailVerificationCommand` sinh token nhưng chưa gửi email — FE đã có nút+API wire).
