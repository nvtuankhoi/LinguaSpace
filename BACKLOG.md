...
- [ ] **Media-upload post type** (L): `CreatePostCommand` nhận `MediaUrls` (max 4) + `PostMetadataDto` shape media/link — nhưng **không có upload endpoint** (avatar command note: Phase 2). Tạm chỉ paste URL. Cần `IStorageService` upload-style endpoint.
