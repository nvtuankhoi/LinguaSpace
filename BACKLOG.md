...
> **Note (2026-07-13):** đã scan api-contract vs FE hệ thống **2 lần** — **các gap FE-only sạch đã cạn hẳn** (mọi ứng viên — badges own-profile, post-edit, avatar — đều đã wire, chỉ là false positive). Backend-only sạch nhất (resend verification sender) đã đóng ở `281dabe`. Còn lại cần **backend feature mới + quyết định contract** (notification preferences, clear-conversation, DM search — endpoint backend chưa có), hoặc **external config/SDK** (FCM cần FCM SDK, Google OAuth cần `GoogleClientId` + Google Identity SDK), hoặc **contract TBD** (VocabCard/Poll/media post types), hoặc **effort lớn video** (screen-share/active-speaker UI). Không còn gap S sẵn sàng làm ngay.
